        ...
        ( z3_check(Sat) ->
            ...
            ( Sat == l_true ->
                ( z3_model(Model),
                  ...
                  writeln('Model Assocs:'), writeln(T1)%,
                %   writeln('Z3 Declarations:'),
                %   z3_declarations(Decls),
                %   writeln(Decls),
                %   z3_model_assocs(Assocs),
                %   writeln(Assocs)   
...
zmi_aux(constr(C), Z3In, CLPQIn, _, Z3Out, CLPQOut) :-
    ...
    writeln('>> Lista dopo normalize_and_expr e conj_to_list (per CLPQ):'),
    writeln(CLPQList),
