...
example_ite1(X, Z) :- constr(ite(X > 3, Z = 1, Z = 0)).  % gestito solo se ite è booleanamente risolubile
example_ite2(X, Z) :- constr(and(X < 10, ite(X < 5, Z = 99, Z = 77))).
example_ite1_fixed(X, Z) :- constr(and(and(X >= 0, X =< 10), ite(X > 3, Z = 1, Z = 0))).
