...
#
# ==========================================================
# Script: CopyTypeErrors_v6.sh
# Autore: Marco Di Ianni
# Descrizione:
#   1) Cerca tutti i file .zmiout che contengono "type error"
#      (case-insensitive) ma NON contengono
#      "ERROR: [Thread main] ----Type error in term:".
#   2) Copia:
...
TARGET_DIR="$1"
OUT_DIR="${2:-$TARGET_DIR/TypeError_Files_Filtered}"
...
# 1️⃣ Trova tutti i .zmiout che contengono "type error"
...
for zmi in "${ZMI_FILES[@]}"; do
  # 2️⃣ Escludi quelli che contengono la forma lunga risolta
  if grep -q "ERROR: \[Thread main\] ----Type error in term:" "$zmi"; then
    ((count_excluded++))
    echo "⏩ Escluso (errore risolto): $(basename "$zmi")"
    continue
  fi
...
echo "   • File esclusi ('Thread main' risolti):   $count_excluded"
echo "   • File copiati:                           $count_copied"
echo "   • Già presenti:                           $count_existing"
echo "   • Mancanti (.smt2/.pl):                   $count_missing"
echo ""
