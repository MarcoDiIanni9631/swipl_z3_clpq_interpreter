...
#
# ==========================================================
# Script: InterpreterAnalysis4.2.sh
...
#   Corregge i path relativi per evitare errori sui moduli.
# ==========================================================
...
process_file() {
  ...
  (
    ...
    timeout ${TIMEOUT_SEC}s "$SWIPL_BIN" --stack-limit=4GB \
      -s "$MAIN_ABS" \
      -g "working_directory(_, '$MAIN_DIR'), load_clean('$FILE_ABS'), set_solver(turibe), zmi(${TARGET}), halt." \
      > "$tmpout" 2>&1
  )
