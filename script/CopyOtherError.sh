...
#
# ==========================================================
# Script: CopyOtherError.sh
# Autore: Marco Di Ianni
# Descrizione:
#   Copia i file .zmiout che contengono "Error" ma NON "Type error",
#   insieme ai rispettivi .smt2 e .smt2.pl.
# ==========================================================
...
TARGET_DIR="$1"
OUT_DIR="${2:-$TARGET_DIR/OtherErrors_Files}"
...
# Trova tutti i file .zmiout che contengono "Error" ma NON "Type error"
mapfile -t ZMI_FILES < <(
  find "$TARGET_DIR" -type f -name "*.zmiout" ! -path "*/OtherErrors_Files/*" | while read -r file; do
    if grep -qi "error" "$file" && ! grep -qi "type error" "$file"; then
      echo "$file"
    fi
  done
)
...
if [ "${#ZMI_FILES[@]}" -eq 0 ]; then
  echo "⚠️ Nessun file .zmiout con 'Error' (esclusi i type error) trovato."
  exit 0
fi
...
echo ""
echo "📊 Riepilogo:"
echo "   • File .zmiout con 'Error' (non type error): $count_errors"
