...
echo "<table>" >> "$HTML_OUT"
echo "<tr><th>File</th><th>SMT Out</th><th>ZMI Result</th><th>Timeout</th><th>MaxDepth</th><th>MaxDepthReached</th><th>ExploredTree</th><th>Z3 Push</th><th>Match</th></tr>" >> "$HTML_OUT"
...
for smtfile in "$DIR"/*.smt2.pl; do
  ...
  for candidate in "$prefix".*.zmiout; do
    ...
    # Selezione prioritaria del risultato
    if [[ "$filename" == *".false"* ]]; then
      ...
      zmiout_file="$candidate"
      label_zmi="timeout"
      # non facciamo break qui: potremmo trovare una classificazione migliore
  ...
  # Calcolo explored_flag dal nome del file selezionato (se presente)
  if [ -n "$zmiout_file" ]; then
    ...
    if [[ "$chosen_name" == *"_totalExplored"* ]]; then
      ...
      explored_flag="?"  # backward compatibility o file prodotti da script vecchi
    fi
  fi
  ...
  # --- LINKS ---
  smt_prefix=$(basename "${smtfile%.smt2.pl}")   # es: chc-comp24-LIA-Lin-001_true
  smt_orig="$PARENT_DIR/$smt_prefix.smt2"

  if [ -f "$smt_orig" ]; then
    link_smt="(<a href=\"../$smt_prefix.smt2\">SMT</a>)"
  else
    link_smt="(missing: $smt_orig)"
  fi
  ...
  if [ -n "$zmiout_file" ]; then
    ...
    link_zmi="(missing: ${prefix}.zmiout)"
  fi
