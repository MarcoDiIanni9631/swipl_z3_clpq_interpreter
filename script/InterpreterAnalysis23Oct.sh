...
# Descrizione:
#   Analizza i file .smt2.pl in una cartella o un singolo file e genera output .zmiout.
...
if [ ! -e "$DIR" ]; then
  echo "❌ File o cartella non trovati: $DIR"
  exit 1
fi
...
# === Esecuzione ===
if [ -d "$DIR" ]; then
  echo "⚙️ Avvio elaborazione parallela su directory: $DIR ..."
  find "$DIR" -type f -name "*.smt2.pl" | parallel -j 8 process_file {}
else
  echo "⚙️ Avvio elaborazione singolo file: $DIR ..."
  process_file "$DIR"
fi
