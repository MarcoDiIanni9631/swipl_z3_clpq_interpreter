...
zmi_aux(Head, Z3In, CLPQIn,SymTabIn, Steps, Z3Out, CLPQOut, SubTree => Head) :-
    ...
    extend_type_tableBody(BodyList, SymTabMid, SymTabMid2),
    writeln('Stampo symbol table dopo inferenziato il body'),
    writeln(SymTabMid2),

    infer_store_types(BodyList, SymTabMid2, SymTabFinal),
    format('📌 SymTab dopo infer store_types: ~w~n', [SymTabFinal]),
    %Head =.. [_|Args],
...
infer_store_types([], SymTab, SymTab).
infer_store_types([constr(C) | Rest], SymTabIn, SymTabOut) :-
    conj_to_list(C, Constraints),
    writeln('Arrivo qui'),
    writeln(Constraints),
    
    infer_store_constraints(Constraints, SymTabIn, SymTabMid),
    infer_store_types(Rest, SymTabMid, SymTabOut).
infer_store_types([_ | Rest], SymTabIn, SymTabOut) :-
    infer_store_types(Rest, SymTabIn, SymTabOut).

infer_store_constraints([], SymTab, SymTab).

infer_store_constraints([Expr | Rest], SymTabIn, SymTabOut) :-
    writeln('🕵️‍♂️ Sto valutando:'),
    writeln(Expr),
    writeln('Symbol table in'),
    writeln(SymTabIn),
    
    (
        % Match: B = store(A, _, _)
        Expr = (B = store(A, _, _)),
        member(A-Type, SymTabIn),
        writeln(SymTabIn),
        Type = array(_, _),
        \+ member(B-_, SymTabIn)
    ->
        format('🧠 Inferring ~w : ~w from store(~w,_,_)~n', [B, Type, A]),
        SymTabMid = [B-Type | SymTabIn]
    ;
        % Match: store(A, _, _) = B (ordine invertito)
        Expr = (store(A, _, _) = B),
        member(A-Type, SymTabIn),
        Type = array(_, _),
        \+ member(B-_, SymTabIn)
    ...
    infer_store_constraints(Rest, SymTabMid, SymTabOut).
