% Automatically generated by smt2map




:- pred new5(int,int,int,int).
:- mode new5(in,in,in,in).
:- pred new4(int,int,int,int).
:- mode new4(in,in,in,in).
:- pred new3(int,int,int,int).
:- mode new3(in,in,in,in).
:- pred new2(int,int,int,int).
:- mode new2(in,in,in,in).
:- pred new1.
:- pred incorrect.
:- pred ff.
:- query ff/0.
:- pred constr(bool).
:- mode constr(in).
:- ignore constr/1.

new5(A,B,C,D) :- constr((and(D>=1,E=0))).
new5(A,B,C,D) :- constr((and(D=< - 1,E=0))).
new4(A,A,B,C) :- new5(A,A,B,C), constr((true)).
new3(A,B,C,D) :- new2(A,B,E,F), 
          constr((and(C>=1,and(G=1,and(F= - 1+D,and(H=1,and(E= - 1+C,I=0))))))).
new3(A,B,C,D) :- new2(A,B,E,F), 
          constr((and(C=< - 1,and(G=1,and(F= - 1+D,and(H=1,and(E= - 1+C,I=0))))))).
new3(A,B,C,D) :- new4(A,B,C,D), constr((and(E=0,and(F=0,C=0)))).
new2(A,B,C,D) :- new3(A,B,C,D), constr((true)).
new1 :- new2(A,B,A,B), constr((true)).
incorrect :- new1, constr((true)).

