...
new11(A,B,C,D,E,F,G,H) :- new8(B,C,D,I,F,G,J), 
          constr((and(A>=1,and(K=1,and(I= - 1+E,and(L=1,and(J=1+H,M=0))))))).
new11(A,B,C,D,E,F,G,H) :- new8(B,C,D,I,F,G,J), 
          constr((and(A=< - 1,and(K=1,and(I= - 1+E,and(L=1,and(J=1+H,M=0))))))).
new10(A,B,C,D,E,F,G) :- new11(H,A,B,C,D,E,F,G), 
          constr((and(D>=1,and(I=0,H=1)))).
new10(A,B,C,D,E,F,G) :- new11(H,A,B,C,D,E,F,G), 
          constr((and(D=<0,and(I=0,H=0)))).
new9(A,B,C,D,E,F,G) :- new10(A,B,C,D,E,F,G), constr((G-A=< - 1)).
new8(A,B,C,D,E,F,G) :- new9(A,B,C,D,E,F,G), constr((true)).
new7(A,B,C,D,E,F,G) :- new6(A,B,C,H,E,I,G), 
          constr((and(B-F>=1,and(J=1,and(H= - 1+D,and(K=1,I=1+F)))))).
new7(A,B,C,D,E,F,G) :- new8(A,B,C,D,E,F,H), constr((and(F-B>=0,H=0))).
