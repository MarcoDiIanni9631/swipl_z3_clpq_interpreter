...
new5(A,B,C,D,E) :- F>=G, F=E, G=A, H=<I, H= 0, I=D, J+ 1=<K, J=D, K=A, L+ 1=<M, 
          N=O, L=C, P=C, N=D, M= 46, read((B,A),O,P), val(map_N,A), val(a,B), 
          val(ghost(0),C), val(ghost(x),D), val(i,E).
new5(A,B,C,D,E) :- F>=G, F=E, G=A, H=<I, H= 0, I=D, J+ 1=<K, J=D, K=A, L>=M+ 1, 
          N=O, L=C, P=C, N=D, M= 46, read((B,A),O,P), val(map_N,A), val(a,B), 
          val(ghost(0),C), val(ghost(x),D), val(i,E).
...
%
verimap(pred_smtvars_types([incorrect,new1('Int','(Array Int Int)','Int','Int','Int'),new2('Int','(Array Int Int)','Int','Int','Int'),new3('Int','(Array Int Int)','Int','Int','Int'),new4('Int','(Array Int Int)','Int','Int','Int'),new5('Int','(Array Int Int)','Int','Int','Int')])).
