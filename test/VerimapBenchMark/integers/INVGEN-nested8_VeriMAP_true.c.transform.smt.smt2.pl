...
new10(A,B,C,D,E,F,G) :- constr((H=0&(I=0&B=0))).
new10(A,B,C,D,E,F,G) :- new7(A,C,D,H,F,G), constr((B>=1&(I=1&(H=1+E&J=0)))).
new10(A,B,C,D,E,F,G) :- new7(A,C,D,H,F,G), constr((B=< - 1&(I=1&(H=1+E&J=0)))).
new9(A,B,C,D,E,F) :- new10(A,G,B,C,D,E,F), 
          constr((B+C-F-E-D=<0&(H=E+D&(I=F+E+D&(J=B+C&G=1))))).
new9(A,B,C,D,E,F) :- new10(A,G,B,C,D,E,F), 
          constr((B+C-F-E-D>=1&(H=E+D&(I=F+E+D&(J=B+C&G=0))))).
new8(A,B,C,D,E,F) :- new9(A,B,C,D,E,F), constr((D-E-F=< - 1&G=E+F)).
new8(A,B,C,D,E,F) :- new5(A,B,G,D,E,F), constr((D-E-F>=0&(H=1&(G=1+C&I=E+F)))).
new7(A,B,C,D,E,F) :- new8(A,B,C,D,E,F), constr((true)).
new6(A,B,C,D,E,F) :- new7(A,B,C,C,E,F), constr((E-C>=1)).
new6(A,B,C,D,E,F) :- new3(A,G,C,D,E,F), constr((C-E>=0&(H=1&G=1+B))).
new5(A,B,C,D,E,F) :- new6(A,B,C,D,E,F), constr((true)).
new4(A,B,C,D,E,F) :- new5(A,B,G,D,E,F), constr((B-E=< - 1&G=0)).
new3(A,B,C,D,E,F) :- new4(A,B,C,D,E,F), constr((true)).
new2(A,B,C,D,E,F) :- new3(A,G,C,D,E,F), constr((E-F=<0&G=0)).
