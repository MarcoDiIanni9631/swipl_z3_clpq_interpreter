...
new8(A,B,C,D,E) :- new2(A,B,C,F,E), constr((B>=1&(F=D+B&G=0))).
new8(A,B,C,D,E) :- new2(A,B,C,F,E), constr((B=<0&(G=1&(F=1+D&H=0)))).
new7(A,B,C,D,E) :- constr((D=<0&F=0)).
new7(A,B,C,D,E) :- new5(A,B,C,F,G), 
          constr((D>=1&(H=1&(F= - 1+D&(I=1&(G=1+E&J=0)))))).
...
new3(A,B,C,D,E) :- new4(A,B,F,D,E), constr((A-C>=1&(G=1&F=1+C))).
new3(A,B,C,D,E) :- new5(A,B,C,D,F), constr((C-A>=0&F=0)).
new2(A,B,C,D,E) :- new3(A,B,C,D,E), constr((true)).
new1 :- new2(A,B,C,D,E), constr((D=0&C=0)).
