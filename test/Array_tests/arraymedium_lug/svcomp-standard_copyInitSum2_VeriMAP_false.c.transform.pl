new3(A,B,C,D,E,F) :- G+ 1=<H, G=F, H=A, I=J+K, J=L, M=F, K=F, N=F, O=P+Q, P=F, 
          Q= 1, read((C,A),M,L), write((B,A),N,I,(R,A)), val(map_N,A), 
          val(a,B), val(b,C), val(ghost(0),D), val(ghost(x),E), val(i,F), 
          val(a,R), val(i,O), new3(A,R,C,D,E,O).
new3(A,B,C,D,E,F) :- G>=H, G=F, H=A, I=<J, I= 0, J=E, K+ 1=<L, K=E, L=A, 
          M+ 1=<N, O=P, M=D, Q=D, O=E, N=R+S, R= 42, S=E, read((C,A),P,Q), 
          val(map_N,A), val(a,B), val(b,C), val(ghost(0),D), val(ghost(x),E), 
          val(i,F).
new3(A,B,C,D,E,F) :- G>=H, G=F, H=A, I=<J, I= 0, J=E, K+ 1=<L, K=E, L=A, 
          M>=N+ 1, O=P, M=D, Q=D, O=E, N=R+S, R= 42, S=E, read((C,A),P,Q), 
          val(map_N,A), val(a,B), val(b,C), val(ghost(0),D), val(ghost(x),E), 
          val(i,F).
...
%
verimap(pred_smtvars_types([incorrect,new1('Int','(Array Int Int)','(Array Int Int)','Int','Int','Int'),new2('Int','(Array Int Int)','(Array Int Int)','Int','Int','Int'),new3('Int','(Array Int Int)','(Array Int Int)','Int','Int','Int')])).
