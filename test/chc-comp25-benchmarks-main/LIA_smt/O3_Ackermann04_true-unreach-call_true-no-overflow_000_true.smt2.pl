...
'main@entry.split' :- 
          constr((and(and(A=true,B=false,C=false,not((2=<D)=E),not((F=<6)=G),H,I,J=or(G,K),K=or(E,L),or(not(M),and(M,N)),not(O),not(P),not(J),M=true,not((2=<Q)=L)),and(H=(O=or(not(Q=<3),not(Q>=0))),I=(P=or(not(D=<23),not(D>=0))))))), 
          'main@entry', ackermann(A,B,C,Q,D,F).
