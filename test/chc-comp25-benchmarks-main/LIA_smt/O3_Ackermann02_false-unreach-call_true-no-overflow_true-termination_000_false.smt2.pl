...
'main@entry.split' :- 
          constr((and(and(A=true,B=false,C=false,not((D=<3)=E),F=or(E,G),H,I,or(not(J),and(J,K)),not(F),J=true,not(L),not(M),not((2=<N)=G)),and(H=(L=or(not(N=<3),not(N>=0))),I=(M=or(not(O=<23),not(O>=0))))))), 
          'main@entry', ackermann(A,B,C,N,O,D).
