...
'main@id.exit.split' :- 'main@tailrecurse.i'(A,B), 
          constr((and(C= - 1+A,and(D=1+B,and(or(not(E),not(F),G=D),and(or(not(E),not(F),H=G),and(or(not(E),not(F),I),and(or(not(J),not(E),K=L),and(or(not(J),not(E),L=M),and(or(not(E),M=(H=20)),and(or(not(E),and(E,F)),and(or(not(J),and(J,E)),and(or(not(J),K),and(or(not(N),and(N,J)),and(N=true,I=(C=0)))))))))))))))).
