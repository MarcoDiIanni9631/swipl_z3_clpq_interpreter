...
'main@verifier.error.split' :- 'main@_bb12'(A,B,C,D,E,F,G,H), 
          constr((I=and(or(J=0,not(K=0)),or(J=0,not(L=0)))&(M=or(not(N),not((F=<O)=P))&(not((H=<E)=Q)&(R=true&(or(not(N),not(S))&(or(not(N),and(N,T))&(or(not(N),I)&(or(not(N),U=ite(V,1,0))&(or(not(N),K=ite(P,1,0))&(or(not(N),O= - 1+G)&(or(not(N),V=(J=0))&(M&(or(not(W),and(N,W))&(or(not(R),and(R,X))&(or(not(X),and(X,Y))&(or(not(Y),and(Y,Z))&(or(not(Z),A1)&(or(not(Z),and(W,Z))&(or(not(Z),A1=(U=0))&(or(not(N),not(B1),not(T))&(B1=and(C1,Q)&not((G=<F)=C1))))))))))))))))))))))).
