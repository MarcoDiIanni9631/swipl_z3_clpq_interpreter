...
new12(A,B,C,D,E) :- new9(F,C,D,E), constr((and(A>=1,and(G=1,and(F=1+B,H=0))))).
new12(A,B,C,D,E) :- new9(F,C,D,E), 
          constr((and(A=< - 1,and(G=1,and(F=1+B,H=0))))).
new11(A,B,C,D) :- new12(E,A,B,C,D), constr((and(B>=1,and(F=1,E=1)))).
new11(A,B,C,D) :- new12(E,A,B,C,D), constr((and(B=<0,and(F=1,E=0)))).
new10(A,B,C,D) :- new11(A,B,C,D), constr((A-C=< - 1)).
new10(A,B,C,D) :- new5(A,E,C,D), constr((and(A-C>=0,and(F=1,E=1+B)))).
new9(A,B,C,D) :- new10(A,B,C,D), constr((true)).
new8(A,B,C,D) :- new7(E,B,C,D), constr((and(C-A>=1,and(F=1,E=1+A)))).
