...
new2(A,B,C,D,E,F,G,H,I,J,K,L,M) :- N>=O, N=A, O=B, P=<Q, P= 0, Q=J, R+ 1=<S, 
          R=J, S=B, T+ 1=<U, V=W, T=I, X=I, V=J, Y=Z, U=H, A1=H, Y=J, 
          read((E,B),W,X), read((D,B),Z,A1), val(i,A), val(size,B), val(a,C), 
...
new2(A,B,C,D,E,F,G,H,I,J,K,L,M) :- N>=O, N=A, O=B, P=<Q, P= 0, Q=J, R+ 1=<S, 
          ...
          read((F,B),W,X), read((C,B),Z,A1), val(i,A), val(size,B), val(a,C), 
          val(b,D), val(a_init,E), val(b_init,F), val(t,G), val(ghost(1),H), 
          val(ghost(0),I), val(ghost(k),J), val(ghost(3),K), val(ghost(2),L), 
          val(ghost(k),M).
new2(A,B,C,D,E,F,G,H,I,J,K,L,M) :- N>=O, N=A, O=B, P=<Q, P= 0, Q=J, R+ 1=<S, 
...
%
verimap(pred_smtvars_types([incorrect,new1('Int','Int','(Array Int Int)','(Array Int Int)','(Array Int Int)','(Array Int Int)','Int','Int','Int','Int','Int','Int','Int'),new2('Int','Int','(Array Int Int)','(Array Int Int)','(Array Int Int)','(Array Int Int)','Int','Int','Int','Int','Int','Int','Int')])).
