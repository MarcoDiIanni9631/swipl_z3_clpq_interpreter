...
new6(A,B,C,D) :- constr((and(E=0,and(F=0,D=0)))).
new5(A,B,C) :- new6(A,B,C,D), constr((and(A-B=<0,D=1))).
new5(A,B,C) :- new6(A,B,C,D), constr((and(A-B>=1,D=0))).
new4(A,B,C) :- new2(A,D,C), constr((D=B+A)).
new3(A,B,C) :- new4(D,B,C), constr((and(C-A>=1,and(E=1,D=1+A)))).
new3(A,B,C) :- new5(A,B,C), constr((A-C>=0)).
new2(A,B,C) :- new3(A,B,C), constr((true)).
new1(A,B,C) :- new2(A,B,C), constr((true)).
incorrect :- new1(A,B,C), constr((and(C=0,and(B=0,A=0)))).
